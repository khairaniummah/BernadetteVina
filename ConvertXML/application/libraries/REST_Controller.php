...
abstract class REST_Controller extends CI_Controller
{
	...
	*/
	 ...
	 */
	public function response($data = array(), $http_code = null, $from, $to)
	{
		global $CFG;
		...
		else
		{
			...
			// If the format method exists, call and return the output in that format
			elseif (method_exists($this->format, 'to_'.$to.'_from_'.$from))
			{
				// Set the correct format header
				header('Content-Type: '.$this->_supported_formats[$to]);

				$output = $this->format->factory($data)->{'to_'.$to.'_from_'.$from}();
			}
